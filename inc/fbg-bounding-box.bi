...
namespace __FBG_NS__
  ...
  destructor BoundingBox() : end destructor
  
  function BoundingBox.centerAt( aX as single, aY as single ) byref as BoundingBox
    dim as single _
      hw = this.width * 0.5f, _
      hh = height * 0.5f
  ...
  private function BoundingBox.overlapVector( bb as BoundingBox ) as Vec2
    return( Vec2( _
      iif( x - bb.x >= 0 andAlso x + width - bb.x <= bb.width, _
        0, iif( bb.x - x < -width, ( bb.x + bb.width ) - ( x + width ), bb.x - x ) ), _
      iif( y - bb.y >= 0 andAlso y + height - bb.y <= bb.height, _
        0, iif( bb.y - y < -height, ( bb.y + bb.height ) - ( y + height ), bb.y - y ) ) ) )
  end function
end namespace
